...
export default function ProcessPage() {
  ...
  return (
    <div className="bg-[#FAFAFA] dark:bg-[#121212] min-h-screen pt-32 pb-24 overflow-hidden transition-colors duration-700">
      ...
      {/* Page Header */}
      <ScrollWrapper className="max-w-7xl mx-auto px-6 md:px-12 text-center mb-24">
        <h2 className="text-[#C5A059] tracking-[0.3em] text-sm font-bold mb-4 uppercase">How We Work</h2>
        <h1 className="text-4xl md:text-5xl font-serif text-gray-900 dark:text-white mb-6 transition-colors duration-700">Our Design & Build Process</h1>
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 relative">
        ...
        {/* The Center Vertical Line (Dark mode adapted) */}
        <div className="hidden md:block absolute left-1/2 top-0 bottom-0 w-[1px] bg-gray-300 dark:bg-gray-800 transform -translate-x-1/2 transition-colors duration-700"></div>

        <div className="space-y-24">
          {steps.map((step, index) => (
            <ScrollWrapper key={index} className={`flex flex-col md:flex-row items-center justify-between w-full ${index % 2 === 0 ? 'md:flex-row-reverse' : ''}`}>
              
              {/* Empty space for the alternating layout */}
              <div className="hidden md:block w-5/12"></div>

              {/* Number Circle in the center */}
              <div className="hidden md:flex w-12 h-12 bg-gray-900 dark:bg-[#1A1A1A] border border-gray-900 dark:border-[#C5A059] text-[#C5A059] font-serif text-xl items-center justify-center rounded-full z-10 shadow-lg absolute left-1/2 transform -translate-x-1/2 transition-colors duration-700">
                {step.number}
              </div>

              {/* Content Card */}
              <div className="w-full md:w-5/12 group cursor-default">
                <div className="bg-white dark:bg-[#1A1A1A] p-8 border border-gray-100 dark:border-gray-800 shadow-sm transition-all duration-500 hover:shadow-xl dark:hover:shadow-[#C5A059]/10 relative overflow-hidden rounded-sm">
                  ...
                  <h3 className="text-2xl font-serif text-gray-900 dark:text-white mb-4 group-hover:text-[#C5A059] dark:group-hover:text-[#C5A059] transition-colors duration-300">
                    {step.title}
                  </h3>
                  ...
                  {/* Step Image */}
                  <div className="w-full h-48 md:h-64 relative overflow-hidden rounded-sm">
                    <div 
      ...
      <ScrollWrapper className="max-w-4xl mx-auto px-6 mt-32 text-center bg-gray-900 dark:bg-black py-16 text-[#F3F2EC] shadow-2xl transition-colors duration-700 rounded-sm border border-gray-800 dark:border-gray-800">
