"use client"; // Important for slider logic & animations
...
export default function HeroSection() {
  ...
  return (
    <section className="relative w-full h-[90vh] flex items-center justify-center text-center overflow-hidden transition-colors duration-700">
      ...
      {/* Premium Light/Dark Overlay */}
      {/* Dark mode mein overlay thoda dark hoga taaki text clear padhne me aaye */}
      <div className="absolute inset-0 bg-[#FAFAFA]/70 dark:bg-[#121212]/80 transition-colors duration-700"></div>
      ...
      >
        ...
        <h1 className="text-5xl md:text-7xl font-serif text-gray-900 dark:text-white leading-tight mb-8 drop-shadow-sm transition-colors duration-700">
          We Design Dreams.<br />
          <span className="font-light italic">We Build Reality.</span>
        </h1>
        <p className="text-gray-700 dark:text-gray-300 text-base md:text-xl max-w-2xl mx-auto mb-12 font-medium leading-relaxed transition-colors duration-700">
          End-to-end luxury architectural and interior solutions tailored for modern living. From concept to creation.
        </p>
        
        {/* Buttons */}
        <div className="flex flex-col md:flex-row items-center justify-center gap-6">
          <Link href="/projects" className="bg-[#C5A059] text-white px-10 py-4 tracking-[0.2em] text-sm md:text-base font-bold hover:bg-gray-900 dark:hover:bg-white dark:hover:text-gray-900 transition-all duration-500 w-full md:w-auto shadow-lg">
            EXPLORE PROJECTS
          </Link>
          <Link href="/contact" className="border-b-2 border-gray-900 dark:border-white text-gray-900 dark:text-white pb-1 tracking-[0.2em] text-sm md:text-base font-bold hover:text-[#C5A059] dark:hover:text-[#C5A059] hover:border-[#C5A059] dark:hover:border-[#C5A059] transition-all duration-300 w-full md:w-auto mt-4 md:mt-0">
            BOOK A CONSULTATION
          </Link>
        </div>
      </motion.div>
        ...
        {slideImages.map((_, index) => (
          <button 
            ...
            className={`h-3 rounded-full transition-all duration-500 ${
              index === currentImage ? 'bg-[#C5A059] w-8' : 'bg-gray-400/60 hover:bg-gray-400 w-3'
            }`}
